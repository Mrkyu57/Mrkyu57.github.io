...
<div style="margin-top: 10px;">
    ...
    <button id="Components" style="padding: 8px 16px; cursor: pointer; background: #555; color: white; border: none; border-radius: 4px; font-weight: bold;">
        Com
    </button>
</div>
...
<script>
  ...
  const comBtn = document.getElementById('Components');
  ...
  let showComponents = false; // 성분 표시 여부 상태
  ...
  function draw() {                                                                           // 그리기 함수 >
    ...
    if (showComponents) {
      const scale = 80; // 벡터 길이 배율
      ctx.setLineDash([5, 5]); // 점선 스타일 설정
      drawMoveVector(position.x, position.y, velocity.x * scale, 0, "blue", "v_x");
      drawMoveVector(position.x + velocity.x * scale, position.y, 0, velocity.y * scale, "green", "v_y");
      drawMoveVector(origin_position.x, origin_position.y, dispX, 0, "#555", "rx");
      drawMoveVector(origin_position.x + dispX, origin_position.y, 0, dispY, "#555", "ry");
